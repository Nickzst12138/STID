...
class DMSTGCN(nn.Module):
    ...
    def forward(self, inputs):
        ...
        x = self.end_conv_2(x)
        output = torch.squeeze(x,dim=-1).transpose(1, 2).reshape(-1,12,10,4)
        return output
...
if __name__ == '__main__':
    ...
    val_loader = DataLoader(MyDataSet(val_data), batch_size=1, shuffle=True, num_workers=0, pin_memory=True)
