...
class DMSTGCN(nn.Module):
    def __init__(self, device, num_nodes, dropout=0.3,
                 out_dim=48, residual_channels=16, dilation_channels=16, end_channels=512,
        ...
        self.start_conv_a = nn.Conv2d(in_channels=4,
        ...
        self.end_conv_1 = nn.Conv2d(in_channels=1888,
                                    out_channels=end_channels,
                                    kernel_size=(1, 1),
                                    bias=True)
        ...
        self.flow_conv = nn.Conv2d(in_channels=end_channels,
                                    out_channels=out_dim,
                                    kernel_size=(1, 1),
                                    bias=True)

        self.speed_conv = nn.Conv2d(in_channels=end_channels,
                                    out_channels=out_dim,
                                    kernel_size=(1, 1),
                                    bias=True)
    ...
    def forward(self, inputs):
        ...
        input: (B, F, N, T)
        """
        time_emb = inputs[:, 0, 1, -2].long()

        inputs = inputs[:,:,:,:-2].transpose(1, 3)
        ind = time_emb.cpu().numpy()
        in_len = inputs.size(3)
        if in_len < self.receptive_field:
            xo = nn.functional.pad(inputs, (self.receptive_field - in_len, 0, 0, 0))
        else:
            xo = inputs
        x = self.start_conv(xo[:,0:4])
        x_a = self.start_conv_a(xo[:, 4:8])
        skip = 0
        ...
        speed = self.speed_conv(x)
        f_output = torch.squeeze(flow,dim=-1).transpose(1, 2).reshape(-1,12,10,4)
        s_output = torch.squeeze(speed, dim=-1).transpose(1, 2).reshape(-1, 12, 10, 4)
...
if __name__ == '__main__':
    ...
    val_loader = DataLoader(MyDataSet(val_data), batch_size=4, shuffle=True, num_workers=0, pin_memory=True)
    adj = np.load('adj_mat.npy')
    device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")
    model = DMSTGCN(device=device, num_nodes=10).cuda()
    for index,(x_sequence, y_flow,y_speed) in enumerate(val_loader):
        ...
        # out_flow, out_speed = model(x_sequence)
